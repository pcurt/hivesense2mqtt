...
COPY --from=package-builder /app/dist/hivesense2mqtt*.whl /app/
RUN pip --python "$(which python)" install --no-cache-dir --no-deps ./hivesense2mqtt*.whl

ENTRYPOINT [ "hivesense2mqtt" ]
